...
exports.Middleware = function(options) {
  ...
  function serve(req, res, file, options) {
    ...
    try {
      ...
      if(!(cached = cache[file]) || cached.mtime.toString() != stat.mtime.toString()) {
        var source = fs.readFileSync(file, 'utf8');
        compiled = AjsVM.compile(source.toString(), options);
        cache[file] = {compiled: compiled, mtime: stat.mtime};
      } else {
