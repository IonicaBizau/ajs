...
class VM extends EventEmitter {
    ...
    constructor (func, opts) {
        ...
        if (this._vmContext_) {
            ...
            this._vmContext_.modInclude = this._moduleInclude.bind(this);
        }
    ...
    _include (request, locals, pLocals) {
        ...
        let filename = path.resolve(pLocals.__dirname, request)
        let template  = null;
     ...
     * @param {Object} pLocals The parent template data.
     */
    _moduleInclude (request, locals, pLocals) {

        if (!request.endsWith(".ajs")) {
            request += ".ajs";
        }

        let filename = path.resolve(VM.ROOT, request)
