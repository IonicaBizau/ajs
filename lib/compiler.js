...
var Compiler = module.exports = function(source, opts) {
  opts = opts || {};

  this.outFunc = opts.outFunc || "_ajs.out";
  this.cbFunc = opts.argFung || "_ajs.cb";
  this.endFunc = opts.endFunc || "_ajs.end";
  this.lineFunc = opts.lineFunc || "_ajs.ln";
...
Compiler.prototype.compile = function() {
  ...
  this.compiled = this._make(this.tree) + "; " + this.endFunc + "();";
...
proto[Node.OUTPUT] = function(output) {
  return this._format([this.outFunc + "(\"" + cleanOutput(output) + "\")"]);
};

proto[Node.EMBED] = function(statements) {
  return this._format([this.outFunc + "(" + cleanEmbed(this._blockStatements(statements).join('')) + ")"]);
}
...
proto[Node.NEW] = function(ctor, args) {
  args = args.length > 0 ? "(" + addCommas(MAP(args, make)) + ")" : "";
...
proto[Node.CALL] = function(func, args, node) {
  ...
  return f + "(" + addCommas(MAP(args, function(expr){
    ...
    if(expr.type == Node.NAME ||
        (expr.type == Node.FUNCTION && (!func.children[0].children ||
          func.children[0].children[0] != '_'))) {
...
proto[Node.FOR] = function(init, cond, step, block) {
  var out = [ "for" ];
  init = (init != null ? this._make(init) : "").replace(/;*\s*$/, ";" + space);
  cond = (cond != null ? this._make(cond) : "").replace(/;*\s*$/, ";" + space);
...
proto[Node.UNARY_PREFIX] = function(operator, expr) {
  var val = this._make(expr);
  if (!(expr.type == Node.NUMBER || (expr.type == Node.UNARY_PREFIX && !HOP(OPERATORS, operator + expr.children[0])) || !this._needsParens(expr)))
    val = "(" + val + ")";
  return operator + (g.is_alphanumeric_char(operator.charAt(0)) ? " " : "") + val;
}
  
proto[Node.UNARY_POSTFIX] = function(operator, expr) {
  var val = this._make(expr);
  if (!(expr.type == Node.NUMBER || (expr.type == Node.UNARY_POSTFIX && !HOP(OPERATORS, operator + expr.children[0])) || !this._needsParens(expr)))
    val = "(" + val + ")";
  return val + operator;
}
...
proto[Node.OBJECT] = function(props) {
  if (props.length == 0)
    return "{}";
  return "{" + MAP(props, function(p){
      if (p.length == 3) {
        ...
        return this[Node.FUNCTION](p[0], p[1][2], p[1][3], p[2]);
      }
      var key = p[0], val = this._make(p[1]);
      if ((typeof key == "number" || +key + "" == key)
           && parseFloat(key) >= 0) {
        key = this[Node.NUMBER](+key);
      } else if (!is_identifier(key)) {
        key = this[Node.STRING](key);
      }
      return this._format([ key + ":", val ]);
    }).join(",") + "}";
}
...
proto[Node.SEQUENCE] = function() {
  return addCommas(MAP(slice(arguments), make));
}
...
Compiler.prototype._blockStatements = function (statements) {
  for (var a = [], last = statements.length - 1, i = 0; i <= last; ++i) {
    ...
    if(this.lineFunc && stat.line != this.line && stat.children[0].type != Node.STRING) {
      code = this.lineFunc+"(" + stat.line + ")\n"  + code;
      this.line = stat.line;
    }
